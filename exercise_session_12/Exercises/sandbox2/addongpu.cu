...
int main () {
  ...
  int numBlocks = nx * ny / BLOCKSIZE + 1;
  ...
  /* TO DO : write cudaMemcpy() instruction below */
   /* We do not really need to copy the two original matrices back */
  cudaMemcpy(mat_in1, mat_in1_gpu, nx * ny * sizeof(float), cudaMemcpyDeviceToHost);
  cudaMemcpy(mat_in2, mat_in2_gpu, nx * ny * sizeof(float), cudaMemcpyDeviceToHost);
